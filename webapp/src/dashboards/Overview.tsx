...
// Включаем русскую локаль для относительных дат (например, «2 часа назад»)
dayjs.locale("ru");
...
export function Overview() {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            // Ограничим тултип топ-5, как в Kibana по умолчанию.
  ...
  const formatSuffix = (timestamp: dayjs.Dayjs | null) => {
    if (timestamp) {
      // Было: "since ${timestamp.fromNow()}"
      return `с ${timestamp.fromNow()}`;
    }
    return undefined;
  };
    ...
    <>
      ...
      <div class="container-fluid">
          ...
          <div class="mt-2 col col-lg-10 col-md-8 col-sm-12">
            <div class="card">
              <div class="card-header d-flex">
                <b>События по типам во времени</b>
          ...
          <div class="mt-2 col col-lg-2 col-md-4 col-sm-12">
            <div class="card">
              <div class="card-header d-flex">
                ...
                >
                  {/* Кнопка-лоадер для выравнивания */}
                  <button
                    class="btn ms-auto"
                    type="button"
                    disabled
        ...
        <div class="row mt-2">
          <div class="col">
            <CountValueDataTable
              title="Топ IP-адресов источников"
              label="IP-адрес"
              rows={topSourceIp.rows}
              loading={topSourceIp.loading}
              searchField="src_ip"
              suffix={formatSuffix(topSourceIp.timestamp)}
              tooltip="На основе событий flow"
            />
          </div>
          <div class="col">
            <CountValueDataTable
              title="Топ IP-адресов получателей"
              label="IP-адрес"
              rows={topDestIp.rows}
